...
// ── Mock receiver (replace with API in Phase 4) ───────────────────────────
const MOCK_RECEIVER = {
  _id: "recv-001",
  name: "Samiha Rahman",
  bloodGroup: "A+",
  location: "Mirpur, Dhaka",
  phone: "+880 1855-332211",
  email: "samiha.rahman@email.com",
  ...
  avatarUrl: null,
  note: "Requires regular transfusion support due to chronic anaemia.",
};
...
export default function ReceiverProfile() {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      // TODO Phase 4: axios.get(`/api/receivers/${id}`)
      setReceiver(MOCK_RECEIVER);
      setAvatarPreview(MOCK_RECEIVER.avatarUrl);
      setLoading(false);
    }, 600);
  ...
  function handleFileChange(e) {
    ...
    // TODO Phase 4: upload via axios.put(`/api/receivers/${id}/avatar`, formData)
    setUploading(true);
      ...
      <main className="max-w-4xl mx-auto pt-20 px-4 pb-24">
        ...
        <div style={glassCard} className="rounded-2xl p-8 md:p-12 relative overflow-hidden">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8 border-t border-[#F0EDE8] pt-8">
            <StatBox
              value={receiver.requestsPlaced}
              label="Requests Placed"
              valueClass="text-3xl font-bold text-[#AF4444]"
            />
            <StatBox
              value={receiver.requestsFulfilled}
              label="Fulfilled"
              valueClass="text-3xl font-bold text-green-600"
            />
            <StatBox
              value={receiver.requestsPlaced - receiver.requestsFulfilled}
              label="Pending"
              valueClass="text-3xl font-bold text-orange-400"
            />
          </div>
...
function InfoItem({ label, value, icon }) {
  return (
    <div className="flex items-start gap-2">
      ...
      <div>
        <p className="text-[9px] uppercase tracking-widest font-bold opacity-40 mb-0.5">
          {label}
        </p>
